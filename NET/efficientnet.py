...
import sys
sys.path.append('/data1/wangwenpeng/Bengali/NET')
from utils import (
    round_filters,
    round_repeats,
    drop_connect,
...
class EfficientNet(nn.Module):
    ...
    def __init__(self, blocks_args=None, global_params=None, in_ch=1):
        super().__init__()
    ...
    @classmethod
    def from_name(cls, model_name, override_params=None, in_ch=1):
        cls._check_model_name_is_valid(model_name)
        blocks_args, global_params = get_model_params(model_name, override_params)
        return cls(blocks_args, global_params, in_ch)

    @classmethod
    def from_pretrained(cls, model_name, num_classes=1000, in_channels=1):
        model = cls.from_name(model_name, override_params={'num_classes': num_classes}, in_ch=in_channels)
        load_pretrained_weights(model, model_name, load_fc=False, ch=in_channels)
        # if in_channels != 3:
...
if __name__ == '__main__':
    ...
    model = EfficientNet.from_name('efficientnet-b0', in_ch=1)
    # stat(model, (3, 224, 224))
    x = torch.zeros((32, 1, 224, 224))
    with torch.no_grad():
